...
[TestClass]
public class SportCarManagerTests
{
    private readonly SportCarManager _carManager = new();

    [TestMethod]
    public void CreateCar_ValidParams_CarIsNotNull()
    {
        var car = _carManager.CreateCar(1, 1);
    ...
    {
        Assert.ThrowsException<ArgumentException>(() => _carManager.CreateCar(tankCapacity, averageFuelRate));
    }
    
    [TestMethod]
    public void GetKmLeft_ReturnsCorrectDistance()
    {
        var car = _carManager.CreateCar(2, 1);
